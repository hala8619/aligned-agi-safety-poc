...
# v11.1 Enhancement: 雑音マップ (Clutter Map - FP削減)
SAFE_CLUTTER_PATTERNS = [
    ...
    # 創作文脈 (v11.1.1追加)
    r'(?i)(?:writing|write)\s+(?:a|an)\s+\w+\s+(?:novel|story|fiction|book)',  # "writing a crime novel"
    r'(?i)(?:writing|write)\s+(?:a|an)\s+(?:novel|story|fiction|book)',  # "writing a novel"
    r'(?i)(?:for|in)\s+(?:my|a|the)\s+(?:novel|story|fiction)',
    # 経済/学術分析 (v11.1.1追加)
...
class V11HybridSystem:
    ...
    def __init__(self, persona: PersonaType = PersonaType.STRICT):
        ...
        self.multi_axis_sum_threshold = 0.8  # 0.5→0.8 (FP削減)
    ...
    def apply_clutter_map(self, query: str, fil_scores: Dict[FILAxis, float]) -> Tuple[Dict[FILAxis, float], bool]:
        """雑音マップ適用 (v11.0から, v11.1.1で減衰強化)"""
        clutter_matches = sum(1 for pattern in SAFE_CLUTTER_PATTERNS if re.search(pattern, query))
        
        if clutter_matches > 0:
            # 動的減衰: max(0.05, 1.0 - matches * 0.5) (v11.1.1: 0.3→0.5に強化)
            attenuation = max(0.05, 1.0 - clutter_matches * 0.5)
